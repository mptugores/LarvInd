...
fcurr <- "03_2_explore_environ_vars_onlyLI_1.r"  # Current script name
...
boxplotEnv (t1, "year", var2plot, ylabs, savePlots=TRUE, path=figpath, saveID="onlyLI_1") # Guardando en .tiff  sin pseudónimo en el nombre del fichero
...
for (i in var2plot){
   png(filename = paste(figpath, "boxplotOttman_", i, "_", faocode, "onlyLI_1.png", sep=""), width = 20, height = 15, units = "cm", res = 100)
...
histAb.environ (t1, var2plot, sp, pa=TRUE, ab=TRUE, savePlots=TRUE, path=figpath, saveID=paste(faocode, "onlyLI_1", sep="_"))
...
for (i in 1:length(var2plot)){
  ...
  savePlots=TRUE, path=figpath, saveID=paste(faocode, "onlyLI_1", bandwidths[i], sep="_"))
}
