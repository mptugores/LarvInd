...
# File for the analysis (fanalisis)
fanalisis <- paste(outdate, "_t_abu_larvind_BFT_allvars.csv", sep="")

# Model name (modelID)
modelID <- "Rev_GAMd_gapyears_8.5mm_v2"   # Revised Delta GAM with emmeans and gapyears
...
# Current script name
fcurr <- "04_model_BFT_11_v6.1_AC_202005_GAMd_emmeans.r"
...
summary_sts <- aggregate(tdef[c("lon", "lat")], by=list(tdef$station_id), mean)
nsampl_station <- aggregate(as.numeric(tdef$station_id), by=list(tdef$station_id), length)
out1 <- merge(summary_sts, nsampl_station, by="Group.1", all.x=TRUE)
write.csv(out1, "summary_samples.csv", row.names=FALSE)


tdef2 <- merge(tdef, nsampl_station, by.x="station_id", by.y="Group.1", all.x=TRUE)
...
# tdef_ori=tdef # to be able to check data.frame before filtering
# tdef=tdef[-which(tdef$gridid2==43),]
# tdef=tdef[-which(tdef$gridid==44),]
# nrow(tdef)

#***
# If filters are applied, create an object to save info into the logfile
# object1 <- list(
# print(paste("Additional filters applied.", sep="")),
# nrow(tdef_ori),
# "tdef=tdef[-which(tdef$gridid==44),]",
# nrow(tdef))

# names(object1) <- c("----------------------", 
#   "nrow before filtering", 
#   "filter 1", 
#   "nrow after filtering")

# En la actualización presentada a ICCAT 2021 no se filtra por gridid


# Additional Filter 1: sin los años 2006 ni 2010 de los gapyears
tdef_ori <- tdef
tdef <- tdef[!(tdef$year==2006 | tdef$year==2010),]
nrow(tdef_ori); nrow(tdef)
...
mbin <- gam(lpres ~ as.factor(year)+s(SALanom, k=3)+s(jd, k=3)+s(tempres2, k=3), family=binomial(link="logit"), method="REML", select=TRUE, data=tdef)
mab <- gam(log(target) ~ as.factor(year)+s(lon, lat, k=9)+s(SALanom, k =3)+s(tempres2, k=3), family=gaussian, method="REML", select=TRUE, data=tabs)
...
# Model responses
#-----------------------
png(paste(modpathid, faocode, "_", modelID, "_pa_", outdate, ".png", sep=""), width=480, height=480*1.5)
par(mfrow=c(3,1))
labsize=1.8
...
plot(mbin, select=1, shade=TRUE, scale=0, rug=TRUE, res=FALSE, cex.lab=labsize, cex.axis=labsize, xlab="Salinity anomaly",ylab="Partial effect")
plot(mbin, select=2, shade=TRUE, scale=0, rug=TRUE, res=FALSE, cex.lab=labsize, cex.axis=labsize, xlab="Day of year", ylab="Partial effect")
plot(mbin, select=3, shade=TRUE, scale=0, rug=TRUE, res=FALSE, cex.lab=labsize, cex.axis=labsize, xlab="Residual temperature", ylab="Partial effect")
dev.off()

png(paste(modpathid, faocode, "_", modelID, "_abund_", outdate, ".png", sep=""), width=480, height=480*1.5)
par(mfrow=c(3,1))
labsize=1.8
...
plot(mab, select=2, shade=TRUE, scale=0, rug=TRUE, res=FALSE, cex.lab=labsize, cex.axis=labsize, xlab="Salinity anmaly", ylab="Partial effect")
plot(mab, select=3, shade=TRUE, scale=0, rug=TRUE, res=FALSE, cex.lab=labsize, cex.axis=labsize, xlab="Residual temperature", ylab="Partial effect")
dev.off()
...
fout3 <- paste(outdate, "_larval_index_outtable_", faocode, 
  "_", max(index_table$year), "_v11_v6.1_", modelID, sep=""); fout3
save.csvxlsx(index_table, path=modpathid, file_out=fout3)

# 6.3.2. Plot larval index

# Plot binomial part
fout4 <- paste(faocode, "_prob_presence_", modelID, "_emmeans", sep=""); fout4
...
# Plot abundance part
fout5 <- paste(faocode, "_cpue_", modelID, "_emmeans", sep=""); fout5
...
if(exists("object1")) print(object1) # Additional filters
if(!exists("object1")) print ("No additional fiters applied.")
cat("\n")
sink()
