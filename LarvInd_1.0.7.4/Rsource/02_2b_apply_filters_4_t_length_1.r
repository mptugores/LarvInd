...
for (i in 1:length(agQ23)) {
  ...
  LR_BFT <- c(0.69, 10.92) # historical length range BFT
  LR_ALB <- c(1.15, 8.50) # historical length range ALB
  
  # Para BFT, substituimos larvas de tallas > 10.92 mm por 10.92 mm
  # if(unique(tt0$spp_name)=="Thunnus_thynnus") tt0$length[tt0$length>11] <- 4.03
  spnum <- process_specifs$sppid [process_specifs$nomcientifico %in% "Thunnus thynnus"]
  if (any(names(length_list)==spnum)){
    ta <- which(names(length_list)==spnum)
    tmp <- length_list[[ta]]
    nbigger <- length(tmp$length[tmp$length > LR_BFT[2]])
    if(nbigger==0) print("No Bluefin tuna larvae is bigger than the historical maximum")
    if(nbigger >0){
      print(paste(nbigger, " Bluefin tuna larvae are bigger than the historical maximum ", sep=""))
      print(paste("and will be replaced by the historical maximum: (", LR_BFT[2], " mm)", sep=""))
      print(tmp[tmp$length > LR_BFT[2],])
    }
    tmp$length[tmp$length > LR_BFT[2]] <- LR_BFT[2] 
    length_list[[ta]] <- tmp
    rm(ta, tmp, nbigger)
  }

  # Para ALB, substituimos larvas de tallas > 8.50 mm por 8.50s mm
  spnum <- process_specifs$sppid [process_specifs$nomcientifico %in% "Thunnus alalunga"]
  if (any(names(length_list)==spnum)){
    ta <- which(names(length_list)==spnum)
    tmp <- length_list[[ta]]
    nbigger <- length(tmp$length[tmp$length > LR_ALB[2]])
    if(nbigger==0) print("No Albacore larvae is bigger than the historical maximum")
    if(nbigger >0){
      print(paste(nbigger, " Albacore larvae are bigger than the historical maximum ", sep=""))
      print(paste("and will be replaced by the historical maximum: (", LR_ALB[2], " mm)", sep=""))
      print(tmp[tmp$length > LR_ALB[2],])
    }
    tmp$length[tmp$length > LR_ALB[2]] <- LR_ALB[2]
    length_list[[ta]] <- tmp
    rm(ta, tmp, nbigger)
  }
